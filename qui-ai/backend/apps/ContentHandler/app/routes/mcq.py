...
async def generate_mcqs(
    ...
    This endpoint starts the MCQ generation process in the background.
    Use the returned session_id to check progress via the status endpoint.
    """
    try:
...
async def _generate_mcqs_background(document_id: str, session_id: str):
    """Background task for MCQ generation"""
    try:
        result = await mcq_service.generate_mcqs_from_document(document_id, session_id)
        logger.info(f"Background MCQ generation completed for session {session_id}")
    except Exception as e:
        logger.error(f"Background MCQ generation failed for session {session_id}: {e}")
